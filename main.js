document.addEventListener('DOMContentLoaded', () => {
  ...
  if (menuBtn && navbar && cartItem && searchForm) {
      menuBtn.onclick = () => {
          navbar.classList.toggle('active');  // Toggle 'active' class on navbar
          // Close other open elements when the menu is toggled
      ...
      if (cartBtn) {
          cartBtn.onclick = () => {
              ...
              // Close other open elements when the cart is toggled
      ...
      if (searchBtn) {
          searchBtn.onclick = () => {
              ...
              // Close other open elements when the search form is toggled
              navbar.classList.remove('active');
              cartItem.classList.remove('active');
          };
      }

      // Optional: Close all elements when clicking outside
      document.addEventListener('click', (event) => {
          if (!event.target.closest('#menu-btn') && !event.target.closest('nav')) {
              navbar.classList.remove('active');
          }
          if (!event.target.closest('#cart-btn') && !event.target.closest('.cart-items-container')) {
              cartItem.classList.remove('active');
          }
          if (!event.target.closest('#search-btn') && !event.target.closest('.search-form')) {
              searchForm.classList.remove('active');
          }
      });
  } else {
